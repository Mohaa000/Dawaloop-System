...
export default function NurseSettings() {
  ...
  const handleAddNurse = async (e) => {
    ...
    setError('');
    try {
      const { uid } = await createAccount({ email, name, role: 'nurse' });
      await setDoc(doc(db, 'staff', uid), { uid, name, email, role: 'nurse', createdAt: serverTimestamp() });
      await sendAccountSetupEmail(email);
      setSetupBanner({ name, email });
      setName(''); setEmail('');
    } catch (err) {
      setError(err.message || 'Failed to create nurse account.');
    } finally {
      setIsCreating(false);
    }
  };
  ...
  return (
    <div>
      <header className="mb-8">
        <h1 className="text-2xl font-bold">Nurses & Settings</h1>
        <p className="mt-1 text-sm text-text-muted">Manage clinic nurse accounts and access.</p>
      </header>
      ...
      <div className="fade-in flex flex-col gap-6">
        {setupBanner && (
          <div className="flex items-start justify-between rounded-card border border-primary-light bg-primary-light p-4">
            <div className="text-sm">
              <div className="font-semibold text-primary-dark">Nurse login created for {setupBanner.name}</div>
              <div className="mt-1">
        ...
        <Card>
          <h3 className="mb-4 font-semibold">Add Nurse</h3>
          {error && <div className="mb-4 rounded-control bg-danger-light p-3 text-sm text-danger">{error}</div>}
          <form onSubmit={handleAddNurse} className="grid grid-cols-1 items-end gap-3 sm:grid-cols-3">
            <div>
            ...
            <Button type="submit" disabled={isCreating} className="h-[42px]">
              <UserPlus size={16} /> {isCreating ? 'Creating…' : 'Add Nurse'}
            </Button>
          </form>
        </Card>
        ...
        <Card padded={false}>
          <div className="border-b border-border p-5">
            <h3 className="font-semibold">Nurse Directory</h3>
          </div>
          <Table>
            <Thead>
              <Th>Name</Th>
            ...
            <tbody>
              {staff.length === 0 ? (
                <tr><td colSpan="4"><EmptyState icon={Users} title="No nurse accounts yet" description="Add your first nurse using the form above." /></td></tr>
              ) : (
                staff.map((member) => (
                  <Tr key={member.id}>
